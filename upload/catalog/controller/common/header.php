...
class Header extends \Opencart\System\Engine\Controller {
	...
	public function index(): string {
		...
		// PoolBuy: surface the seller portal only to customers who are actually linked
		// to a seller profile. Guarded so that disabling or removing the extension
		// restores stock behaviour - the header renders on every page, so a missing
		// extension must never be able to break the site.
		$data['poolbuy_seller'] = '';

		if ($this->config->get('module_poolbuy_status') && $this->customer->isLogged() && is_file(DIR_EXTENSION . 'poolbuy/catalog/model/poolbuy/seller.php')) {
			$this->load->model('extension/poolbuy/poolbuy/seller');

			if ($this->model_extension_poolbuy_poolbuy_seller->getSellerByCustomerId((int)$this->customer->getId())) {
				$data['poolbuy_seller'] = $this->url->link('extension/poolbuy/seller', 'language=' . $this->config->get('config_language'));
			}
		}
